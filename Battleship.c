...
typedef struct Player {
    char name[100];
    int guesses;
    char enemyField[FIELD_SIZE][FIELD_SIZE];
...
void saveScore(Player* player, Player* enemy);
...
int main() {
   ...
   //play until one of the players is dead
    ...
    //if player 1 is alive he has won
    Player* winner;
    Player* looser;
    if (alive(&p1)) {
        winner = &p1;
        ...
        winner = &p2; //player 1 is not alive, therefor player 2 has won
    }
...
void createNewPlayer(Player* player) {
    player->guesses = 0;
    //go through every position of the field and sets it value to 0
    //we need to do that, because the memory is just allocated and not initialized therefor the field contains some "random" values
...
void manualPlacement(Player* player) {
    ...
    do {
        ...
        //if the input of the player is valid, the ship is placed successfully
        int x1;
        int x2;
        int y1;
        int y2;
...
void saveScore(Player* winner,Player* looser) {
    ...
    fprintf(scoreFile, "%s has won with a score of: %i against %s\n", winner->name, winner->guesses, looser->name);
    fclose(scoreFile);
}
